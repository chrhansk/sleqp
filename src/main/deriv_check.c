...
restore_iterate(SleqpDerivCheckData* data, SleqpIterate* iterate)
{
...
static SLEQP_RETCODE
set_check_iterate(SleqpDerivCheckData* data, bool* reject)
{
  ...
  SLEQP_CALL(
    sleqp_sparse_vector_reserve(sleqp_iterate_get_func_grad(check_iterate),
                                func_grad_nnz));

  SLEQP_CALL(
    sleqp_sparse_vector_reserve(sleqp_iterate_get_cons_val(check_iterate),
                                cons_val_nnz));

  SLEQP_CALL(
    sleqp_sparse_matrix_reserve(sleqp_iterate_get_cons_jac(check_iterate),
                                cons_jac_nnz));
...
static double
get_perturbation(const SleqpDerivCheckData* data,
                 ...
                 int j)
{
  ...
  double base_perturbation
    = sleqp_params_get(params, SLEQP_PARAM_DERIV_PERTURBATION);
...
static SLEQP_RETCODE
create_perturbed_unit_direction(SleqpDerivCheckData* data,
                                const SleqpIterate* iterate,
                                const int j,
                                double* perturbation)
{
...
static SLEQP_RETCODE
compute_hessian_cons_products(SleqpDerivCheckData* data, int j)
{
  SleqpProblem* problem = data->problem;
  ...
  {
    SLEQP_CALL(create_unit_direction(data->multipliers, k));
...
static SLEQP_RETCODE
compute_hessian_products(SleqpDerivCheckData* data,
...
static SLEQP_RETCODE
create_check_iterate(SleqpDerivCheckData* data,
                     const SleqpIterate* iterate,
                     const int j,
                     double* perturbation)
{
  ...
  SLEQP_CALL(create_perturbed_unit_direction(data, iterate, j, perturbation));
...
static SLEQP_RETCODE
eval_at_check_iterate(SleqpDerivCheckData* data, SLEQP_DERIV_CHECK flags)
{
  SleqpIterate* check_iterate = data->check_iterate;
  SleqpProblem* problem       = data->problem;
  ...
  SLEQP_CALL(
    sleqp_problem_eval(problem, NULL, func_ptr, func_grad, cons_val, cons_jac));

  if (func_ptr)
  {
    SLEQP_CALL(sleqp_iterate_set_func_val(check_iterate, func_val));
  }
...
static SLEQP_RETCODE
check_deriv_func_first_order(SleqpDerivCheckData* data,
                             SleqpIterate* iterate,
                             int j,
                             double perturbation)
{
  ...
  const double tolerance
    = sleqp_params_get(data->params, SLEQP_PARAM_DERIV_TOL);

  const double func_diff = sleqp_iterate_get_func_val(check_iterate)
                           - sleqp_iterate_get_func_val(iterate);

  const double actual_value = func_diff / perturbation;

  const double expected_value
    = sleqp_sparse_vector_value_at(sleqp_iterate_get_func_grad(iterate), j);
...
static SLEQP_RETCODE
check_deriv_cons_first_order(SleqpDerivCheckData* data,
                             SleqpIterate* iterate,
                             int j,
                             double perturbation)
{
  ...
  for (int i = 0; i < num_constraints; ++i)
  {
    ...
    const double lower_value
      = sleqp_sparse_vector_value_at(sleqp_iterate_get_cons_val(iterate), i);

    const double upper_value
      = sleqp_sparse_vector_value_at(sleqp_iterate_get_cons_val(check_iterate),
                                     i);
...
static SLEQP_RETCODE
check_deriv_func_second_order(SleqpDerivCheckData* data,
                              SleqpIterate* iterate,
                              int j,
                              double perturbation)
{
  ...
  SLEQP_CALL(
    sleqp_sparse_vector_add_scaled(sleqp_iterate_get_func_grad(iterate),
  ...
  SLEQP_CALL(
    sleqp_sparse_vector_scale(data->hessian_estimate, 1. / perturbation));
  ...
  for (int k = 0; k < num_variables; ++k)
  {
    ...
    const double actual_value
      = sleqp_sparse_vector_value_at(data->hessian_estimate, k);
...
static SLEQP_RETCODE
check_deriv_cons_second_order(SleqpDerivCheckData* data,
                              SleqpIterate* iterate,
                              int j,
                              double perturbation)
{
...
static SLEQP_RETCODE
compute_combined_cons_grad(SleqpDerivCheckData* data,
                           SleqpSparseVec* multipliers,
                           SleqpIterate* iterate,
...
static SLEQP_RETCODE
check_deriv_simple_second_order(SleqpDerivCheckData* data,
                                SleqpIterate* iterate,
                                int j,
                                double perturbation)
{
  ...
  // Compute Hessian product estimate
  {
    ...
    SLEQP_CALL(
      compute_combined_cons_grad(data,
                                 multipliers,
                                 check_iterate,
                                 data->combined_cons_grad_check_iterate));

    SLEQP_CALL(
      sleqp_sparse_vector_add_scaled(data->combined_cons_grad_iterate,
                                     data->combined_cons_grad_check_iterate,
                                     -1.,
                                     1.,
                                     0,
  ...
  for (int k = 0; k < num_variables; ++k)
  {
    ...
    const double expected_value
      = sleqp_sparse_vector_value_at(data->hessian_prod, k);
...
static SLEQP_RETCODE
check_deriv(SleqpDerivCheckData* data,
            SLEQP_DERIV_CHECK flags,
            SleqpIterate* iterate,
            int j,
            double perturbation)
{
  if (flags & SLEQP_DERIV_CHECK_FIRST_FUNC)
  {
    SLEQP_CALL(check_deriv_func_first_order(data, iterate, j, perturbation));
  }

  if (flags & SLEQP_DERIV_CHECK_FIRST_CONS)
  {
    SLEQP_CALL(check_deriv_cons_first_order(data, iterate, j, perturbation));
  }

  if (flags & SLEQP_DERIV_CHECK_SECOND_FUNC)
  {
    SLEQP_CALL(check_deriv_func_second_order(data, iterate, j, perturbation));
  }

  if (flags & SLEQP_DERIV_CHECK_SECOND_CONS)
  {
    SLEQP_CALL(check_deriv_cons_second_order(data, iterate, j, perturbation));
  }

  if (flags & SLEQP_DERIV_CHECK_SECOND_SIMPLE)
  {
    SLEQP_CALL(check_deriv_simple_second_order(data, iterate, j, perturbation));
  }
...
static SLEQP_RETCODE
eval_and_check_deriv(SleqpDerivCheckData* data,
                     SleqpIterate* iterate,
                     SLEQP_DERIV_CHECK flags,
                     int j)
{
  ...
  {
    SLEQP_CALL(compute_hessian_products(data, flags, iterate, j));

    SLEQP_CALL(eval_at_check_iterate(data, flags));

    SLEQP_CALL(check_deriv(data, flags, iterate, j, perturbation));
  }
