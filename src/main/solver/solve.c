...
check_feasibility(SleqpSolver* solver, bool* feasible)
{
  ...
  *feasible = sleqp_iterate_is_feasible(iterate,
                                        feasibility_residuum,
                                        feas_eps);
...
static SLEQP_RETCODE
run_solving_loop(SleqpSolver* solver,
                 ...
                 double time_limit)
{
  ...
  bool enable_restoration = sleqp_options_get_bool(solver->options,
  ...
  while(true)
  {
    ...
    double remaining_time = sleqp_timer_remaining_time(solver->elapsed_timer,
                                                       time_limit);

    SleqpProblemSolver* problem_solver = get_problem_solver(solver);

    SLEQP_CALL(sleqp_problem_solver_set_iteration(problem_solver,
                                                  solver->iterations));
    ...
    SLEQP_PROBLEM_SOLVER_STATUS status = sleqp_problem_solver_get_status(problem_solver);

    assert(status != SLEQP_PROBLEM_SOLVER_STATUS_UNKNOWN);
    assert(status != SLEQP_PROBLEM_SOLVER_STATUS_RUNNING);

    solver->iterations += sleqp_problem_solver_elapsed_iterations(problem_solver);

    if(!unlimited_iterations)
    {
      remaining_iterations -= sleqp_problem_solver_elapsed_iterations(problem_solver);
    }
                                 ...
                                 double time_limit)
{
  ...
  SLEQP_CALL(run_solving_loop(solver,
                              max_num_iterations,
                              time_limit));
