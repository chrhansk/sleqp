...
create_restoration_primal(SleqpSolver* solver)
{
  ...
  SLEQP_CALL(sleqp_restoration_problem_transform(solver->problem,
...
{
  SleqpIterate* restoration_iterate = sleqp_problem_solver_get_iterate(solver->restoration_problem_solver);

  SLEQP_CALL(sleqp_restoration_problem_restore(solver->problem,
...
create_restoration_solver(SleqpSolver* solver)
{
  ...
  const int num_variables = sleqp_problem_num_variables(solver->restoration_problem);
...
SLEQP_RETCODE sleqp_solver_toggle_phase(SleqpSolver* solver)
{
  ...
  if(current_phase == SLEQP_SOLVER_PHASE_OPTIMIZATION)
  {
    ...
    SLEQP_CALL(sleqp_problem_solver_set_primal(solver->restoration_problem_solver,
                                               solver->restoration_primal));
  ...
  else
  {
    ...
    SLEQP_CALL(sleqp_problem_solver_set_primal(solver->problem_solver,
                                               solver->primal));
