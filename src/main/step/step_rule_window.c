...
static int data_index(const StepRule* step_rule, int index)
{
  if(step_rule->len == 0)
  {
    return 0;
...
static int data_begin(const StepRule* step_rule)
{
  return step_rule->curr - step_rule->len + 1;
}

static int data_end(const StepRule* step_rule)
{
  return step_rule->curr;
}

static int compute_ref_index(const StepRule* step_rule)
{
...
static double compute_historic_reduction_ratio(const StepRule* step_rule,
                                               const double iterate_merit,
                                               const double trial_exact_merit,
                                               const double model_reduction)
{
  const double current_reduction_ratio = (iterate_merit - trial_exact_merit) / model_reduction;
...
static SLEQP_RETCODE add_accepted_step(StepRule* step_rule,
                                       ...
                                       double model_reduction)
{
  ...
  const double eps = sleqp_params_get(step_rule->params,
                                      SLEQP_PARAM_EPS);
...
static SLEQP_RETCODE step_rule_window_apply(double iterate_merit,
                                            ...
                                            void* step_data)
{
  ...
  const double historic_reduction_ratio = compute_historic_reduction_ratio(step_rule,
                                                                           iterate_merit,
                                                                           trial_exact_merit,
                                                                           model_reduction);

  *reduction_ratio = SLEQP_MAX(current_reduction_ratio,
  ...
  *accept_step = (*reduction_ratio >= accepted_reduction);
  ...
  sleqp_log_debug("Step with current reduction ratio %e, historic reduction ratio %e, accepted: %s",
  ...
  if(*accept_step)
  {
    SLEQP_CALL(add_accepted_step(step_rule,
                                 iterate_merit,
                                 model_reduction));
  }
...
static SLEQP_RETCODE step_rule_window_reset(void* step_data)
{
  StepRule* step_rule = (StepRule*) step_data;
...
static SLEQP_RETCODE step_rule_window_free(void* step_data)
{
  StepRule* step_rule = (StepRule*) step_data;
                                            ...
                                            int window_size)
{
  ...
  SleqpStepRuleCallbacks callbacks =
    {
      .rule_apply = step_rule_window_apply,
