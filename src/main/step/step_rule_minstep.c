...
static SLEQP_RETCODE step_rule_init(StepRule* step_rule,
...
{
  const double total_model_reduction = step_rule->model_decrease_sum_ref + model_reduction;

  return (step_rule->ref_exact_merit - trial_exact_merit) / total_model_reduction;
}

static SLEQP_RETCODE step_rule_minstep_apply(double iterate_merit,
                                             ...
                                             void* step_data)
{
  ...
  *accept_step = (*reduction_ratio >= accepted_reduction);
  ...
  {
    sleqp_log_debug("Found new minimum with merit %e",
                    iterate_merit);
  ...
  {
    sleqp_log_debug("Found new local maximum with merit %e",
                    iterate_merit);
...
static SLEQP_RETCODE step_rule_minstep_reset(void* step_data)
{
  StepRule* step_rule = (StepRule*) step_data;
...
static SLEQP_RETCODE step_rule_minstep_free(void* step_data)
{
  StepRule* step_rule = (StepRule*) step_data;
                                             ...
                                             int step_count)
{
  ...
  SleqpStepRuleCallbacks callbacks =
    {
      .rule_apply = step_rule_minstep_apply,
