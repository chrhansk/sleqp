                     ...
                     SLEQP_BFGS_SIZING sizing)
{
  ...
  *block = (BFGSBlock){0};
  ...
  {
    SLEQP_CALL(
      sleqp_sparse_vector_create_empty(block->point_diffs + i, dimension));

    SLEQP_CALL(
      sleqp_sparse_vector_create_empty(block->grad_diffs + i, dimension));

    SLEQP_CALL(
      sleqp_sparse_vector_create_empty(block->point_products + i, dimension));
  ...
  SLEQP_CALL(
    sleqp_sparse_vector_create_empty(&(block->inner_cache), dimension));

  SLEQP_CALL(sleqp_sparse_vector_create_empty(&(block->prod_cache), dimension));
            ...
            SleqpOptions* options)
{
  ...
  *data = (BFGS){0};
  ...
  SLEQP_CALL(sleqp_sparse_vector_create_full(&data->grad_diff, num_variables));

  SLEQP_CALL(sleqp_sparse_vector_create_full(&data->point_diff, num_variables));
  ...
  SLEQP_CALL(
    sleqp_sparse_vector_create_full(&data->block_point_diff, num_variables));

  SLEQP_CALL(sleqp_sparse_vector_create_full(&data->prod_cache, num_variables));

  SLEQP_CALL(
    sleqp_sparse_vector_create_full(&data->block_direction, num_variables));

  SLEQP_CALL(sleqp_sparse_vector_create_full(&data->block_prod, num_variables));
                   ...
                   double* initial_scale)
{
  ...
  SLEQP_CALL(sleqp_sparse_vector_dot(grad_diff, point_diff, &grad_point_dot));
...
bfgs_compute_products(BFGSBlock* block, const SleqpParams* params)
{
  ...
  for (int val = begin; val <= block->curr; ++val)
  {
    ...
    SLEQP_CALL(sleqp_sparse_vector_dot(product, direction, &bidir_product));
    ...
    {
      SLEQP_CALL(
        sleqp_sparse_vector_copy(current_grad_diff, current_damped_grad_diff));
    }
    ...
    if (!is_damped)
    {
      ...
      sleqp_num_assert(
        sleqp_sparse_vector_eq(block->prod_cache, block->grad_diffs[i], eps));
    }
#endif
  }
                ...
                const SleqpSparseVec* grad_diff)
{
  ...
  SLEQP_CALL(sleqp_sparse_vector_copy(point_diff, block->point_diffs[next]));

  SLEQP_CALL(sleqp_sparse_vector_copy(grad_diff, block->grad_diffs[next]));

  if (block->sizing != SLEQP_BFGS_SIZING_NONE)
  {
    block->point_diff_inner_dots[next]
      = sleqp_sparse_vector_norm_sq(point_diff);
          ...
          void* data)
{
  ...
  for (int i = 0; i < num_blocks; ++i)
  {
    ...
    {
      SLEQP_CALL(
        sleqp_sparse_vector_push(bfgs->block_point_diff,
    ...
    {
      SLEQP_CALL(
        sleqp_sparse_vector_push(bfgs->block_grad_diff,
    ...
    const double point_normsq
      = sleqp_sparse_vector_norm_sq(bfgs->block_point_diff);
               ...
               void* data)
{
  ...
  for (int i = 0; i < num_blocks; ++i)
  {
    ...
    else
    {
      ...
      {
        SLEQP_CALL(
          sleqp_sparse_vector_push(bfgs->block_direction,
