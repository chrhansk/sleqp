...
static SLEQP_RETCODE log_rosenbrock_val(SleqpFunc* func,
                                        double* func_val,
                                        void* func_data)
{
  *func_val = log(inner);
...
static SLEQP_RETCODE log_rosenbrock_grad(SleqpFunc* func,
...
static SLEQP_RETCODE log_rosenbrock_hess_prod(SleqpFunc* func,
                                              ...
                                              void* func_data)
{
  SLEQP_CALL(sleqp_sparse_vector_reserve(product, log_rosenbrock_num_variables));

  const double dx = sleqp_sparse_vector_value_at(direction, 0);
  const double dy = sleqp_sparse_vector_value_at(direction, 1);

  const double hxx = (-40000.*(y - sq(x)) + 80000.*sq(x) + 2) / inner - sq(-40000.*x*(y - sq(x)) - 2*sq(1 - x)) / sq(inner);

  const double hxy = (-40000.*x) / inner - (20000.*(y - sq(x))*(-40000.*x*(y - sq(x)) - 2*(1 - x))) / sq(inner);
  ...
  if(func_dual)
  {
    SLEQP_CALL(sleqp_sparse_vector_push(product,
                                        0,
                                        (*func_dual) * hxx*dx + hxy*dy));

    SLEQP_CALL(sleqp_sparse_vector_push(product,
                                        1,
                                        (*func_dual) * hxy*dx + hyy*dy));
  }
...
void log_rosenbrock_setup()
{
  ...
  ASSERT_CALL(sleqp_sparse_vector_create(&log_rosenbrock_var_lb,
                                         2,
                                         2));
  ...
  ASSERT_CALL(sleqp_sparse_vector_create(&log_rosenbrock_var_ub,
                                         2,
                                         2));
