...
START_TEST(test_sparse_matrix_vector_product)
{
  ...
  ASSERT_CALL(sleqp_sparse_matrix_create(&matrix, num_rows, num_cols, nnz_max));
  ...
  ASSERT_CALL(sleqp_sparse_matrix_vector_product(matrix, vector, result));
...
START_TEST(test_sparse_reserve)
{
  ...
  ASSERT_CALL(sleqp_sparse_matrix_create(&matrix, size, size, 0));
...
START_TEST(test_sparse_increase_size)
{
  ...
  ASSERT_CALL(
    sleqp_sparse_matrix_create(&matrix, initial_size, initial_size, 0));
...
START_TEST(test_sparse_pop_column)
{
  ...
  ASSERT_CALL(sleqp_sparse_matrix_create(&matrix, size, size, size));
