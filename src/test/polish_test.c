...
polish_setup()
{
  ...
  {
    ASSERT_CALL(
      sleqp_sparse_vector_from_raw(sleqp_iterate_get_cons_val(iterate),
                                   values,
                                   num_constraints,
                                   zero_eps));
  }
...
START_TEST(test_polish_inactive_vars)
{
  ...
  ASSERT_CALL(
    sleqp_working_set_add_variable(working_set, 0, SLEQP_ACTIVE_LOWER));

  // zero dual and inactive => removed
  ASSERT_CALL(
    sleqp_working_set_add_variable(working_set, 1, SLEQP_ACTIVE_BOTH));
...
START_TEST(test_polish_zero_dual_vars)
{
  ...
  ASSERT_CALL(
    sleqp_working_set_add_variable(working_set, 2, SLEQP_ACTIVE_UPPER));

  ASSERT_CALL(
    sleqp_sparse_vector_push(sleqp_iterate_get_vars_dual(iterate), 0, -1.));
...
START_TEST(test_polish_inactive_cons)
{
  ...
  // zero dual and inactive => removed
  ASSERT_CALL(
    sleqp_working_set_add_constraint(working_set, 1, SLEQP_ACTIVE_BOTH));
...
START_TEST(test_polish_zero_dual_cons)
{
  ...
  ASSERT_CALL(
    sleqp_working_set_add_constraint(working_set, 2, SLEQP_ACTIVE_UPPER));

  ASSERT_CALL(
    sleqp_sparse_vector_push(sleqp_iterate_get_cons_dual(iterate), 0, -1.));
