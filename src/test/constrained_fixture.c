...
static SLEQP_RETCODE func_free(void* data)
{
...
void constrained_setup()
{
  ...
  ASSERT_CALL(sleqp_alloc_array(&func_data->duals, constrained_num_constraints));
  ASSERT_CALL(sleqp_alloc_array(&func_data->direction, constrained_num_variables));
